...
def generate_heatmap(height, width, channel, pt, sigma):
    heatmap = np.zeros((channel, height, width))
    num_pts, _ = pt.shape
    assert num_pts == channel
    for i in range(channel):
        heatmap[i, int(pt[i, 1]), int(pt[i, 0])] = 1
        heatmap[i] = cv2.GaussianBlur(heatmap[i], sigma, 0)
        am = np.amax(heatmap[i])
        heatmap[i] /= am / 255
    return heatmap

def generate_vector(height, width, kpts, vec_pair, theta):
    channel = len(vec_pair[0])
    vector = np.zeros((2*channel, height, width))
    cnt = np.zeros((channel, height, width))
    for i in range(channel):
        a = vec_pair[0][i]
        b = vec_pair[1][i]
        ax = kpts[a][0] * 1.0 
...
#         print(ax, ay, bx, by)
        for h in range(min_h, max_h):
            for w in range(min_w, max_w):
                ...
                if dis <= theta:
#                     print(h, w, dis)
                    vx, vy = vector[2*i, h, w], vector[2*i+1, h, w]
                    origin_cnt = cnt[i, h, w]
                    vector[2 * i, h, w] = (vx * origin_cnt + bax) / (origin_cnt + 1)
                    vector[2 * i + 1, h, w] = (vy * origin_cnt + bay) / (origin_cnt + 1)
                    cnt[i, h, w] = origin_cnt + 1
    return vector
